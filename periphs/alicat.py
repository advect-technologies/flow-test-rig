...
class AlicatBaseDF(utils.PeriphDF):
    ...
    def generate(cls, unit_id: str):
        return cls(unit_id=unit_id, pressure=round(gauss(1), 2))
...
class AlicatBase:
    ...
    @classmethod
    def _mock_persist_data(cls, unit_id: str, data: dict[str, str | float]):
        p = Path(f"periphs/.mock/{unit_id}.json")
        if not p.exists():
            p.parent.mkdir(exist_ok=True)
...
class AlicatFlowController(AlicatBase):
    ...
    async def write_setpoint(
        self, value: float, units: AlicatFlowUnits | None = None
    ) -> bool:
