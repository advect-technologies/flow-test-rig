...
class FlowTestApp(App):
        ...
        with TabbedContent(initial="live"):
                ...
                with Horizontal():
                    with Vertical(classes="panel"):
                        ...
                        with Vertical():
                            ...
                            self.progress_static = Static(
                                "🟡 No active test", id="progress-display", markup=True
                            )
                            yield self.progress_static
                            with Center():
                                self.progress_bar = ProgressBar(
                                    id="test-progress-bar", total=100, show_eta=False
                                )
            ...
            with TabPane("🎛️ Control", id="control"):
                with Vertical():
                    ...
                    self.status_banner = Static(
                        "", id="status-banner", classes="banner"
                    )
                    yield self.status_banner

                    with Horizontal(id="button-bar", classes="panel"):
                        self.start_btn = Button(
                            "▶️ START TEST", id="start_test", variant="success"
                        )
                    ...
                    with Horizontal():
                        with Vertical(classes="panel", id="metadata-panel"):
                            ...
                            self.height = Input(
                                placeholder="height", type="number", id="height"
                            )
                            yield self.height

                            yield Label("Cylinder Diameter (cm):")
                            self.diameter = Input(
                                placeholder="diameter", type="number", id="diameter"
                            )
                            yield self.diameter

                            yield Label("Gas:")
                            self.gas = Select.from_values(
                                ["Air", "N2", "CO2", "Other"], value="Air", id="gas"
                            )
                            yield self.gas
                            ...
                            self.notes = TextArea(
                                placeholder="Flow characterization...", id="notes"
                            )
                            yield self.notes
                            ...
                            self.protocol_select = Select(
                                ...
                                value=self._scan_protocols()[0][1]
                                if self._scan_protocols()
                                else Select.NULL,
                                allow_blank=True,
                                id="protocol-select",
                            )
                        ...
                        with Vertical(classes="panel", id="control-panel"):
                            ...
                            yield Button(
                                "Tare Scale", id="tare_scale", variant="primary"
                            )
    ...
    def watch_test_status_data(self, status: dict):
        ...
        stage_info = (
            f"Stage {status.get('current_stage', '?')}/{status.get('stage_total', '?')}"
        )
        ...
        lines = (
            f"🟢 [bold]{status.get('name', 'Test')}[/bold] — {stage_info}",
    ...
    def on_protocol_selected(self, event: Select.Changed):
        ...
        if isinstance(event.value, Path):
            test_rig_event_q.put_nowait(models.ProtocolChangedEvent(file=event.value))
